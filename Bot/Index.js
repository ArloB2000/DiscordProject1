...
//json added the true/false inputs as strings and i'm too lazy to parse it all to booleans if i can just do a funny with the quotation marks
	...
	if (msg.guild.id != null) {
		...
		//possible link detection system i made. here's my rednecky way of extracting the link from the message and displaying it
		//if(LogLinks == true){
		//	var m = msg.content;
			//if the link is currently being slurped up or not
		//	var LinkStatus = false;
			//the link detected
		//	var link;
			//doesn't work for some reason so basically gay
			// for(var i = 0; i < m.length; i++){
			// 	var j = m.CharAt(i), k, l, n, o;
			// 	if(LinkStatus = false){
			// 		if(j == 'h'){
			// 			k = m.CharAt(i + 1);
			// 		}
			...
			// 			link += "" + j;
			// 		}
					//logs the link in the console, saves it in the logs file, and breaks out of the loop.
			//		else{
			//			console.log("LINK DETECTED: " + link);
			//			lM.push({timestamp, message: msg.author.tag + " " + link});
			//			let data = JSON.stringify(lM);
			//			fileL.set(timestamp + msg.author.tag, "" + data + "");
			//			fileL.save();
			//			break;
			//		}
			//	}
